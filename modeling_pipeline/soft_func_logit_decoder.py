#!/usr/bin/env python3
"""
Soft function-logit decoder for BAHD and UGT datasets.

This model keeps the full per-residue function logits and replaces hard argmax
tokens with soft token embeddings:
  logits [B, L, 8, 260] -> softmax -> expected token embeddings -> transformer -> task head
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a soft function-logit decoder.")
    ...
    parser.add_argument(
        "--output_dir",
        default=str(SCRIPT_DIR / "results" / "soft_func_logit_decoder"),
...
def collate_soft_logits(batch: Sequence[Tuple[np.ndarray, np.ndarray]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    sequences, labels = zip(*batch)
    max_len = max(seq.shape[0] for seq in sequences)
    x = np.zeros((len(sequences), max_len, 8, 260), dtype=np.float32)
...
class SoftFuncLogitDecoder(nn.Module):
    def __init__(
        self,
        d_model: int,
        n_heads: int,
        ...
        self.channel_dim = d_model // 8
        self.token_embeddings = nn.Parameter(torch.randn(8, 260, self.channel_dim) * 0.02)
    ...
    def forward(self, logits: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        expected = torch.einsum("blkv,kvd->blkd", probs, self.token_embeddings)
        hidden = expected.reshape(expected.shape[0], expected.shape[1], -1)
...
def train_model(
    ...
    loader = DataLoader(
        ...
        shuffle=True,
        collate_fn=collate_soft_logits,
    )

    model = SoftFuncLogitDecoder(
        d_model=d_model,
        n_heads=n_heads,
        n_layers=n_layers,
        dropout=dropout,
...
def score_model(
    ...
    loader = DataLoader(dataset, batch_size=len(dataset), shuffle=False, collate_fn=collate_soft_logits)
    model = SoftFuncLogitDecoder(
        d_model=d_model,
        n_heads=n_heads,
        n_layers=n_layers,
...
def fit_full_model(
    ...
    trained["model_type"] = "soft_func_logit_decoder"
...
def run_family_task(
    ...
    sequences = build_sequence_feature_list(dataset_dir, cache, device=resolved_device, flatten=False)
    ...
    metrics_payload = {
        ...
        "feature_type": "function_logits_sequence_softmax_decoder",
    ...
    if save_models:
        ...
        torch.save(model_bundle, run_dir / f"{stem}_models.pt")
