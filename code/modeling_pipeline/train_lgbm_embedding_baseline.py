#!/usr/bin/env python3
"""
LightGBM classifier baseline for BAHD and UGT datasets using ESM3
...
The only major feature change is:
    ...
    model:
        one binary LGBMClassifier per substrate label
...
Important: this script keeps the same manual one-label-at-a-time loop used by
train_lr_baseline.py and train_lr_embedding_baseline.py, including the
constant-model fallback for single-class training folds.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a LightGBM baseline from mean-pooled ESM3 last-layer embeddings."
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--output_dir",
        default=str(SCRIPT_DIR / "results" / "lgbm_embedding_baseline"),
    ...
    parser.add_argument("--n_estimators", type=int, default=DEFAULT_LGBM_PARAMS["n_estimators"])
    parser.add_argument("--learning_rate", type=float, default=DEFAULT_LGBM_PARAMS["learning_rate"])
    parser.add_argument("--num_leaves", type=int, default=DEFAULT_LGBM_PARAMS["num_leaves"])
    parser.add_argument("--max_depth", type=int, default=DEFAULT_LGBM_PARAMS["max_depth"])
    parser.add_argument("--min_child_samples", type=int, default=DEFAULT_LGBM_PARAMS["min_child_samples"])
    parser.add_argument("--subsample", type=float, default=DEFAULT_LGBM_PARAMS["subsample"])
    parser.add_argument("--colsample_bytree", type=float, default=DEFAULT_LGBM_PARAMS["colsample_bytree"])
    parser.add_argument("--reg_alpha", type=float, default=DEFAULT_LGBM_PARAMS["reg_alpha"])
    parser.add_argument("--reg_lambda", type=float, default=DEFAULT_LGBM_PARAMS["reg_lambda"])
    parser.add_argument(
        "--write_predictions",
...
def terminal_underscore_token(value: str | Path) -> str:
    """Return the final underscore-delimited token from an ID.

    Examples:
        Aann_UGT25_PWA63700.1 -> PWA63700.1
        Abel_AbUGT1_UGT1_QGI57841.1 -> QGI57841.1
    """
    text = strip_known_feature_suffix(value)
    if not text:
        return ""
...
def add_accession_candidate_keys(keys: List[str], value: str | Path | None) -> None:
    """
    Add accession-only fallback keys.

    This handles cases where UGT embedding filenames contain the same terminal
    accession as labels.pt but use a different prefix. The normal resolver still
    detects duplicate matches, so accession-only matching is safe as long as the
    accession is unique in the embedding directory.
    """
    if value is None:
        return

    token = terminal_underscore_token(value)
    if not token:
        return

    _add_candidate_key(keys, token)

    # Also try the no-version accession form: PWA63700.1 -> PWA63700
    if "." in token:
        _add_candidate_key(keys, token.split(".", 1)[0])
...
def build_embedding_index(embedding_dir: Path, embedding_glob: str) -> Dict[str, List[Path]]:
    ...
    for path in sorted(embedding_dir.glob(embedding_glob)):
        stem = strip_embedding_suffix(path)
        accession = terminal_underscore_token(stem)
        accession_no_version = accession.split(".", 1)[0] if "." in accession else accession
...
def resolve_embedding_path_for_sample(
    *,
    ...
    # Accession-only fallback. This is useful when the embedding filename
    # contains the same terminal accession but a different family/enzyme prefix.
    for raw in (fasta_id, enzyme_id, id_hint or ""):
        add_accession_candidate_keys(candidate_keys, raw)
...
# -----------------------------------------------------------------------------
# LightGBM and metrics. Kept intentionally similar to train_lr_baseline.py.
# -----------------------------------------------------------------------------

def build_lgbm_params(args_or_params: Any | None = None, random_seed: int = 1) -> Dict[str, Any]:
    """Build LightGBM parameters from defaults plus optional argparse namespace/dict."""
    params = dict(DEFAULT_LGBM_PARAMS)

    if args_or_params is not None:
        source = vars(args_or_params) if hasattr(args_or_params, "__dict__") else dict(args_or_params)
        for key in [
            "n_estimators",
            "learning_rate",
            "num_leaves",
...
def make_model(lgbm_params: Mapping[str, Any]) -> Any:
    if LGBMClassifier is None:
        raise ImportError(
            "lightgbm is not installed. Install it with `pip install lightgbm` "
            "before running train_lgbm_embedding_baseline.py."
        ) from LIGHTGBM_IMPORT_ERROR
    return LGBMClassifier(**dict(lgbm_params))
...
def fit_binary_model(X: np.ndarray, y: np.ndarray, lgbm_params: Mapping[str, Any]) -> Dict[str, Any]:
    """Fit one binary LightGBM model, with the same constant fallback as the old script."""
    classes = np.unique(y)
    if classes.size < 2:
        return {
            "mode": "constant",
    ...
    return {
        ...
        "constant": None,
        "model": model,
        "classes": [int(c) for c in getattr(model, "classes_", classes).tolist()],
    }


def score_binary_model(model_bundle: Dict[str, Any], X_test: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    if model_bundle["mode"] == "constant":
        constant = int(model_bundle["constant"])
        pred = np.full((X_test.shape[0],), constant, dtype=np.uint8)
        score = np.full((X_test.shape[0],), float(constant), dtype=np.float32)
        return pred, score

    model = model_bundle["model"]
    pred = model.predict(X_test).astype(np.uint8, copy=False)
    ...
    if hasattr(model, "predict_proba"):
        ...
        score = proba[:, pos_idx].astype(np.float32, copy=False)
    elif hasattr(model, "decision_function"):
        score = model.decision_function(X_test).astype(np.float32, copy=False)
    else:
        score = pred.astype(np.float32, copy=False)
...
def run_cv_multilabel(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(cv_splits):
        ...
        fold_params["random_state"] = int(random_seed) + int(fold_idx)

        for label_idx in range(n_labels):
            model = fit_binary_model(X_train, y_train[:, label_idx], fold_params)
...
def fit_full_models(
    ...
    models: Dict[str, Any] = {}
    for label_idx, label_name in enumerate(label_names):
        models[str(label_name)] = fit_binary_model(X, y[:, label_idx], lgbm_params)
    return {
...
def run_family_task(
    ...
    if pickle_models:
        model_bundle = fit_full_models(X, y, label_names, lgbm_params=lgbm_params)
...
def main() -> None:
    ...
    lgbm_params = build_lgbm_params(args, random_seed=args.random_seed)
